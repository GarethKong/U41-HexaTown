...
namespace Mkey
{
    ...
    public class SoundMaster : MonoBehaviour
    {
        ...
        [Space(8, order = 0)] [Header("Basic audio clips", order = 1)] [SerializeField]
        private AudioClip music;

        [SerializeField] private AudioClip buttonClick;
        [SerializeField] private AudioClip openWindow;
        [SerializeField] private AudioClip closeWindow;

        [SerializeField] private AudioClip[] listAudioClips;

        #endregion basic clips

        [SerializeField] private uint audioSoucesMaxCount = 10;
        ...
        private string saveNameVibration = "mk_vibrationon";
        ...
        [SerializeField] private float musicVolumeMult = 0.1f;
        ...
        public virtual void SetVibration(bool on)
        {
            VibrationOn = on;
            if (saveSettings) PlayerPrefs.SetInt(saveNameVibration, (VibrationOn) ? 1 : 0);
        }
        ...
        public void SoundPlayClipAtPos(float playDelay, AudioClip aC, Vector3 pos, float volumeMultiplier,
            Action callBack)
        {
        ...
        protected IEnumerator PlayClipAtPoint(float playDelay, AudioClip aC, Vector3 pos, float volumeMultiplier,
        ...
        public void PlayCurrentMusic()
        {
            ...
            if (MusicOn)
            {
                if ((currentMusic == musicSource.clip) && musicSource.isPlaying) // check volume
                {
                    ...
                    if (vol != volume)
                        musicVolumeTween = SimpleTween.Value(gameObject, vol, volume, 1f)
        ...
        public void StopMusic()
        {
            ...
            if (musicSource && musicSource.isPlaying)
            {
                musicVolumeTween = SimpleTween.Value(gameObject, Volume * musicVolumeMult, 0.0f, 1f)
                    .SetOnUpdate((float val) => { musicSource.volume = val; }).AddCompleteCallBack(() =>
                    {
                        musicSource.Stop();
                        musicSource.volume = 0;
                    }).ID;
